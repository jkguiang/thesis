...
Alongside the forces, there is the Higgs mechanism, carried by the Higgs boson, which is responsible for endowing the fundamental particles with mass (Section~\ref{sec:higgs}). 
...
Of course, a rigorous description of the SM could only be properly treated alongside a stack of textbooks. 
...
Unfortunately, four to five years of attentive instruction and rigorous study across multiple subjects---including quantum mechanics, special relativity, and a bevy of additional mathematical formalism---are required to even begin reading QFT textbooks, and well over a lifetime may be required to fully understand them\footnotemark{}. 
...
\begin{figure}[htb]
    \centering
    \subfloat[]{\includegraphics[width=0.2\textwidth]{fig/feynman/vertices/qed_vertex_2.pdf}\label{fig:qed1}}\qquad
    \subfloat[]{\includegraphics[width=0.2\textwidth]{fig/feynman/vertices/ewk_vertex_Vff.pdf}\label{fig:weak}}\qquad
    \subfloat[]{\includegraphics[width=0.2\textwidth]{fig/feynman/vertices/ewk_vertex_WWZ.pdf}\label{fig:ewk1}}\qquad
    \subfloat[]{\includegraphics[width=0.2\textwidth]{fig/feynman/vertices/ewk_vertex_VVVV.pdf}\label{fig:ewk2}}\\
    \subfloat[]{\includegraphics[width=0.2\textwidth]{fig/feynman/vertices/qcd_vertex_gqq.pdf}\label{fig:qcd1}}\qquad
    \subfloat[]{\includegraphics[width=0.2\textwidth]{fig/feynman/vertices/qcd_vertex_ggg.pdf}\label{fig:qcd2}}\qquad
    \subfloat[]{\includegraphics[width=0.2\textwidth]{fig/feynman/vertices/qcd_vertex_gggg.pdf}\label{fig:qcd3}}\\
    \subfloat[]{\includegraphics[width=0.2\textwidth]{fig/feynman/vertices/hig_vertex_HXX.pdf}\label{fig:hig1}}\qquad
    \subfloat[]{\includegraphics[width=0.2\textwidth]{fig/feynman/vertices/hig_vertex_HHHH.pdf}\label{fig:hig2}}\qquad
    \subfloat[]{\includegraphics[width=0.2\textwidth]{fig/feynman/vertices/hig_vertex_HHVV.pdf}\label{fig:hig3}}
...
\begin{figure}[htb]
    \centering
    \subfloat[$\Pep+\Pem\to\PGg$]{\includegraphics[width=0.25\textwidth]{fig/feynman/vertices/qed_vertex_elec_1.pdf}\label{fig:ee_to_g}}\quad
    \subfloat[$\Pem+\PGg\to\Pem$]{\includegraphics[width=0.25\textwidth]{fig/feynman/vertices/qed_vertex_elec_2.pdf}\label{fig:eg_to_e}}\quad
    \subfloat[$\Pem\to\PGg+\Pem$]{\includegraphics[width=0.25\textwidth]{fig/feynman/vertices/qed_vertex_elec_0.pdf}\label{fig:e_to_ge}}
...
Consider two electrons barrelling towards each other. 
...
\footnotetext{
    In fact, the answer to this question is expressed in the units of ``barns,'' literally as in ``hitting the broadside of a barn,'' coined by Manhattan Project scientists during World War II~\cite{BarnOrigin}. 
}
...
\begin{equation}
    \langle|\mathcal{M}|^2\rangle = \frac{2g_e^4}{(p_1 \cdot p_3)^2(p_1 \cdot p_4)^2[(p_1 \cdot p_2)^4 + (p_1 \cdot p_3)^4 + (p_1 \cdot p_4)^4]}
\end{equation}
...
By integrating the ``amplitude'' $\mathcal{M}$ over the phase space---effectively, the different ways one can apportion the available energy amongst the particles in the final state of the system---one can finally obtain the cross-section.
This exercise, and the compact answer above, serves to demonstrate the sheer complexity of QFT calculations. 
Many details were left out beyond the computation itself, including details surrounding the spin of the electron (e.g. the quoted amplitude is ``spin-averaged''), the definition of spin, the definition of four-momentum, all of special relativity, and so on. 
Those details, and much more, can be found in Chapter 7 of the illustrious textbook from which this entire example was borrowed~\cite{GriffithsParticle} alongside a few years of physics education at an accredited educational institution. 
...
\footnotetext{Fields are used here in a quantum context (scalar fields in particular) but there are many classical examples: the temperature of a room, the height of the sea, the strength of a magnetic field, i.e. quantities that may be different at every position in space and may also vary in time.}
...
\begin{equation}
    \mathcal{L} = \underbrace{\frac{1}{2}(\partial_\mu\phi)(\partial^\mu\phi)}_{\text{kinetic term}} - \underbrace{\frac{1}{2}\big(\frac{mc}{\hbar}\big)^2\phi^2}_\text{mass term}
\end{equation}
represents a spin-0 particle with mass $m$. 
As in classical mechanics, plugging the appropriate Lagrangian density into the Euler-Lagrange equation yields a precise description of the dynamics of fundamental particles. 
...
Dark matter is therefore an enormous missing piece---27\% of the universe~\cite{PlanckDM}---in the SM.
...
But this expansion is accelerating, and we do not know why. 
More precisely, we are left wondering: where does the enormous amount of energy required to drive this acceleration come from? 
With no known answer to this question, we have yet another invisible quantity---this time, a ``dark'' energy that constitutes 68\% of the universe~\cite{PlanckDM}---as a placeholder.
...
\end{itemize}
Answers to many of the questions above could lie in more precise measurements of the properties of the Higgs boson, such as those presented in this dissertation.