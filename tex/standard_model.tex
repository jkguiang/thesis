...
The entire universe is, we believe, the emergent picture from an elegant, infinite dance of fundamental particles. 
...
% Together, these three forces (EM, weak, strong) and the Higgs mechanism---and gravity, which a topic left for another time---describe the totality of the operation of the \textit{everything}, from the sight of the sun in the sky, to the fusion reaction causing the sun to shine, to the formation of the sun and all of the other stars in the universe, to the first instance of creation itself. 
% Thankfully, however, it can be expressed on a blackboard through ``Feynman diagrams.''
\subsection{Feynman diagrams}
Fortunately, there is way to write down essential Standard Model calculations in a simple drawing, so-called ``Feynman diagrams,'' which, consequently, help experimentalists keep track of physically allowed processes. 
...
These vertices act as building blocks that can be fit together (e.g. Fig.~\ref{fig:ee_scattering}) according to a small set of rules that correspond to real physical laws. 
Thus, any physical process can be represented with a Feynman diagram. 
...
This can be represented as a Feynman diagram (Fig.~\ref{fig:ee_scattering}) which is assembled by two EM vertices. 
...
While this question is entirely quantum mechanical at the scale of electrons, the classical picture of two colliding objects is still useful\footnotemark{}, where this question is answered by computing the cross-sectional area presented by one object to the other. 
\footnotetext{In fact, the answer to this question is expressed in the units of ``barns,'' literally as in ``hitting the broad side of a barn,'' coined by during World War II.}
