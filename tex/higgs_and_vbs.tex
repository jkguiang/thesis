...
\footnotetext{This is well-motivated: dark matter is known to have mass, and it may have obtained that mass through the Higgs mechanism.}
...
\begin{figure}[htb]
    ...
    \caption[An illustration of two quarks hadronizing into two distinct jets.]{
        An illustration of two quarks hadronizing into two distinct jets based on Figs. 8.1, 8.2, and 8.3 in Ref.~\cite{GriffithsParticle}.
    }
...
If the B meson were much lighter, its decay products would be more columnated, and the secondary vertex could therefore only be reconstructed if the tracker were infinitely precise. 
...
\begin{figure}[htb]
    ...
    \caption[An illustration of a generic boson with a small, moderate, and large Lorentz boost, respectively, decaying to two jets.]{
        An illustration of a generic boson with a small, moderate, and large Lorentz boost, respectively, decaying to two jets.
        As the boson receives a larger boost, its hadronic decay products become increasingly columnated until they are reconstructed as a single merged jet. 
...
\begin{figure}[htb]
    ...
    \caption[VBS jets signature]{
        ...
        \Mjj is the invariant mass of the two VBS jets and $\abs{\detajj}$ is the psuedorapidity (defined in Appendix~\ref{apdx:cms_coords}) gap between them. 
    }
...
\begin{figure}[htb]
    \centering
    \subfloat{\includegraphics[width=0.45\textwidth]{fig/vbswh/fireworks/signal/vbswh_evt131107_rz.png}}
    \qquad
    \subfloat{\includegraphics[width=0.45\textwidth]{fig/vbswh/fireworks/bkg/ttbar_rz.png}}
...
This is often done with the \MGvATNLO ``matrix-element'' generator~\cite{Alwall:2014hca}, which finds the relevant Feynman diagrams, including very rare ones, and randomly generates different realizations of the final state based on an approximation of the underlying theory. % TODO: cite MadGraph AMC@NLO paper
...
Next, a ``general-purpose'' generator (e.g. \PYTHIA or \POWHEG~\cite{Sjostrand:2014zea,Alioli:2010xd,Frixione:2007vw}) takes the event generated by the previous step\footnotemark{} and promotes it to something closer to what we would observe in nature. % TODO: cite Pythia and POWHEG papers
...
\begin{figure}[htb]
    ...
    \caption[A generic ABCD configuration]{
        A generic ABCD configuration where selections on the statistically independent quantities $x$ and $y$ are used to form the signal region (region A), and three ``control'' regions (regions B, C, and D) used to estimate the background in the signal region. 
        In the toy example shown, the method closes exactly for background (blue): $B\times C/D = 15\times4/10 = 6$.
    }
    \label{fig:abcd}
\end{figure}
...
Our task can now be expressed in the proper language of statistics: determine the most probable value of $\mu$ given $n$, $s$, and $b$ is given by calculating maximum of the likelihood $L(\mu)$. 
