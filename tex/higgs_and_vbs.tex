...
Importantly, many SM particles decay to neutrinos, which pass through CMS completely undetected, and it is possible that BSM particles are similarly missed, but the presence of these partices can be inferred if the reconstruction is performed with high precision. 
...
Jets are constructed using the anti-\kt algorithm~\cite{Cacciari:2008gp, Cacciari:2011ma}, which clusters PF candidates according to a configurable ``distance parameter,'' which determines the size of the jet cone. % TODO: add a diagram of a cone in CMS coordinates
