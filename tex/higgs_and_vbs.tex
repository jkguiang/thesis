...
Jets are constructed using the anti-\kt algorithm~\cite{Cacciari:2008gp, Cacciari:2011ma}, which clusters deposits in the calorimeters according to a configurable ``distance parameter,'' which corresponds to the size of the jet cone. % TODO: maybe explain deltaR = sqrt(dphi^2 + deta^2) and have a diagrame of a cone in CMS coordinates
...
For CMS analysis, a deep neural network called \DeepJet was trained to use the presence of a secondary vertex, along with additional kinematic information about the jet and its constituents, to ``tag'' jets as originating from a \Pb quark. 
...
\subsubsection{Merged jets}
In cases where a highly energetic particle decays to a pair of quarks, however, the resulting jets overlap or are near enough to each other to be difficult to separate. 
...
% First, we try to ``reconstruct'' each particle produced in a given pp collision, namely leptons and jets. 
% Then, we develop an optimized selection strategy, using only simulated signal and background events to start, that picks out pp collision events containing the final state of our signal. 
...
% Finally, we take the expected signal count $S$ and predicted background count $B$ and compare them to the number of real pp collisions that pass our selections. 
