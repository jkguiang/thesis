...
\begin{figure}[htb]
    ...
    \caption[Event display for a simulated VBS WH signal event]{
        Event display for a simulated VBS WH signal event. 
        The event is shown in the x-y plane (left) and x-z plane (right), and the final-state particles (thick red lines) are labeled. 
...
Because it is simulated all of the way to the trigger decisions, the simulation can be passed through the PF algorithm and all other data postprocessing steps performed on data (Fig.~\ref{fig:mcgen_diagram}). 
...
\begin{figure}[htb]
    ...
    \caption[The likelihood with the Gaussian approximation of the likelihood overlayed]{
        The likelihood with the Gaussian approximation of the likelihood overlayed for different values of $n$, showing poor agreement for small $n$ (left), decent agreement for $n = 56$, and good agreement for $n = 550$.
    }
